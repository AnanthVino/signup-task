...
const StepOne = () => {
  ...
  const save = () => {
    ...
    localStorage.setItem("page", "/admin/steptwo");
    window.location.href = "/admin/steptwo";
  }
  ...
  return (
    <div className="container d-flex flex-column justify-content">
      <form >
        ...
        <div className="form-group">
          <label>Email</label>
          <input type="text" className="form-control input-box rounded-0" defaultValue={localStorage.getItem("email")} onChange={(evt) => updateEmail( evt.target.value )} placeholder="Email" required/>
        </div>
      </form>
      <div className="d-flex justify-content-between">
        <a href="/" className="btn btn-primary rounded-0">Go Back</a>
        <button onClick={save} className="btn btn-primary rounded-0">Next Step</button>
      </div>
    </div>
  );
}
